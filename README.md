...
一个面向 Codex 的中文项目开发 Skill。它把需要持久追踪的开发工作变成仓库里可检查的资料：当前项目规格、变更依据、实施任务、测试证据，以及会影响后续工作的长期记忆。

根目录 `README.md` 只供人阅读，不是 Skill 运行时输入。可安装的 Skill 包位于 `skills/project-lifecycle/`；安装时选择这个子目录，README、测试和仓库资料不会进入安装目录。

运行时入口保持短小，详细规则按需从 Skill 包内的 `references/` 读取。项目资料按“当前事实”和“活动变更”组织；流程按风险收缩，不要求每个小修复都走重型仪式。
...
小修复可以只在聊天里给出短设计并直接实现；需要跨会话追踪的普通功能保留必要资料，高风险工作才使用完整变更流程。

用户不需要操作这张流程图。直接用自然语言开始、确认、继续或查询状态即可。Skill 会读取项目工件，判断当前阶段，并在有助于决策或接力时给出简短方向：
...
| 部分 | 职责 |
| --- | --- |
| `skills/project-lifecycle/` Skill 包 | 通用规则、阶段参考、初始化器和确定性脚本，任何项目都可以复用 |
...
因此当前保持一个编排 Skill：`skills/project-lifecycle/SKILL.md` 只放共同规则，进入某个阶段时才读取对应 `references/*.md`。将来只有某个能力能够独立安装、独立触发、独立完成时，才把它拆成单独 Skill。
...
下面的文件路径均相对于 `skills/project-lifecycle/`；根目录 README、测试和 Git 仓库资料不属于 Skill 包。

| 文件 | 解决的问题 |
| --- | --- |
| `SKILL.md` | 运行时底线、风险路由和按需加载入口；不承载完整方法论。 |
| `agents/openai.yaml` | 让 Codex UI 能识别 Skill，并提供中文默认调用入口。 |
| `references/rules.md` | 定义全局底线、项目常驻规范、工作项约束的边界、优先级和上下文压缩恢复顺序。 |
| `references/workflow.md` | 详细的自然语言执行、恢复、阶段门槛和可选接力协议。 |
...
如果工作需要跨会话追踪或属于高风险开发且项目还没有 `.agent/`，Skill 会先初始化项目资料目录；已有源码、测试、Git 和项目文档不会被覆盖。然后 Agent 会读取相关代码、规格和匹配的长期记忆，只追问会改变范围、行为、约束或验收的问题。
...
从你的视角，标准主线就是“需求出现 → 讨论 → 设计 → 开发 → 测试 → 沉淀”。表中的方案、详细设计和实施计划是高风险工作的检查点；普通功能可以合并或省略不必要的阶段，并把决定写入当前工件。
...
这个行为完整写在 [`references/workflow.md`](./skills/project-lifecycle/references/workflow.md)，它是详细的执行与接力协议。已有编号的工作项在状态变化、阻塞或需要跨会话时，Agent 再给出：
...
单文件、低风险、行为边界清楚的小修复，可以只在聊天里确认目标和最小思路，随后实现并验证，不创建完整变更目录。需要持久追踪的普通功能在 `requirements.md` 标记 `workflow: compact`，只保留必要资料；新项目、跨模块修改、公共接口变化、迁移、安全或架构调整再走完整流程。
...
| 层级 | 适用范围 | 典型内容 | 保存位置 |
| --- | --- | --- | --- |
| 全局底线 | 所有项目、所有受管理任务 | 尊重当前语言、先查状态、明确批准、不覆盖资料、不虚报测试、控制漂移 | Skill 的 `SKILL.md` |
...
上下文压缩后不依赖聊天记忆。每次恢复按固定顺序读取 `AGENTS.override.md`、`AGENTS.md`、`.agent/rules/always.md`、状态查询 JSON、当前工作项 `requirements.md` 和本阶段批准工件；需要时再用“当前 / 本次 / 下一步”简报。因此即使上一轮的规则没有留在上下文里，项目文件仍是可恢复的事实源。
