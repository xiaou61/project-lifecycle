...
工作项约束可以收紧项目规范，不能静默放宽项目规范；提案、设计和任务只负责授权本次实现的方案与步骤。项目规范缺失、尚未得到用户确认、冲突或需要例外时，先停下并说明，不靠上下文记忆猜测。初始化不会替项目写入固定规则；首次初始化后先检查仓库并向用户提出规则草案，确认后再创建或更新 `.agent/rules/always.md`。详细优先级和恢复顺序见 [references/rules.md](references/rules.md)。
...
3. 读取 `.agent/rules/always.md`；初始化后缺失是等待确认的正常状态，缺失、空白或存在冲突时先报告并向用户确认，不能直接进入实现；文件中的“待项目确认”也必须在用户确认后才能生效。
...
当用户明确使用本 Skill 管理重要工作且项目没有 `.agent/` 时，先用一句话说明会增加项目资料但不会修改源码，也不会替项目确定常驻规则，然后运行：
