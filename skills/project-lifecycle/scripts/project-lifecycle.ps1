...
param(
    ...
    [ValidateSet("init", "status", "resume", "validate", "history", "updates", "record", "checkpoint", "push-check")]
    [string]$Command,
...
if ($Command -eq "push-check") { $forwardedArgs += "--push-check" }
